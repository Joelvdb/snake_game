...
class Bomb:
    def __init__(self, time_state, gd, location: list = None):
        self.gd = gd
    ...
    def __draw_white(self, loc):
        """draws the color white on previous locations"""
        while loc != []:
            for j in loc:
                self.__location.remove(j)
